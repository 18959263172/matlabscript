    ...
    mclOutputHandlerFcn print_handler)
{
  ...
  if (!mclmcrInitialize())
    return false;
  if (!GetModuleFileName(GetModuleHandle("dlltest"), path_to_dll, _MAX_PATH))
    return false;
    {
...
{
  return dlltestInitializeWithHandlers(mclDefaultErrorHandler, mclDefaultPrintHandler);
}
...
{
  return mclFeval(_mcr_inst, "sd", nlhs, plhs, nrhs, prhs);
}

LIB_dlltest_C_API 
bool MW_CALL_CONV mlfSd()
{
  return mclMlfFeval(_mcr_inst, "sd", 0, 0, 0);
}
